    ...
    const openInSpotifyApp = useCallback(() => {
        ...
        setTimeout(() => {
            if (isModalOpen) {
                ...
                spotifyWindowRef.current = window.open(
        ...
        <>
            <div className="play-card-button-container">
                <button
                    ...
                    title={
                        duration
                            ? `Play (${Math.floor(duration / 60)}:${(duration % 60).toString().padStart(2, "0")})`
                            : "Play on Spotify"
                    }
                >
                    <div className="play-card-icon-container">
                        <span className="play-card-icon">{isLoading ? "⌛" : "▶"}</span>
                    </div>
                </button>
            </div>
            ...
            {isModalOpen && trackId && (
                <div className="spotify-modal-overlay" onClick={closeSpotifyModal}>
                    <div className="spotify-modal-content" onClick={(e) => e.stopPropagation()}>
                        <button className="spotify-modal-close" onClick={closeSpotifyModal}>
                            ×
                        </button>
